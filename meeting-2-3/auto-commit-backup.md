...
5. Сделай первый коммит: `git add -A && git commit -m "initial backup"`.
...
#### 6.2 Сохраняем доступы в одно надёжное место

1. Создай локальный файл **вне всех бэкап-папок**: `~/.secrets/github_credentials.md` (Windows: `%USERPROFILE%\.secrets\github_credentials.md`) с шаблоном:
   ```
   # GitHub
   Email: <email>
   Username: <username>
   Пароль: ВПИШИ_СЮДА_САМ
   2FA recovery codes: ВСТАВЬ_СЮДА_ПОСЛЕ_НАСТРОЙКИ
   ```
2. На macOS/Linux: `chmod 600` на файл. Скажи пользователю открыть файл и вписать пароль руками (не диктовать в чат).
3. Если GitHub потребует двухфакторку (2FA) — помоги настроить через приложение-аутентификатор и напомни сохранить recovery-коды в этот же файл.
4. Проверь: папка `~/.secrets/` НЕ входит в список бэкапируемых папок из Шага 1 и не лежит внутри них.
